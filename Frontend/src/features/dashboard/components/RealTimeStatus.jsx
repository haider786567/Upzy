...
const RealTimeStatus = ({ monitors = [] }) => {
  const getStatusStyle = (status) => {
    switch (status) {
      ...
      default: return 'bg-primary/10 text-primary/50 border-primary/20';
    }
  };
              ...
              <tr key={i} className="hover:bg-white/3 transition-colors group cursor-default">
                ...
                <td className="px-8 py-6">
                  <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border ${getStatusStyle(monitor.status)} shadow-lg shadow-black/20`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${getStatusDot(monitor.status)} ${monitor.status === 'UP' ? 'animate-pulse' : ''}`} />
                    <span className="text-[10px] font-bold uppercase tracking-wider">{monitor.status === 'UP' ? 'Operational' : monitor.status}</span>
                  </div>
                </td>
                <td className="px-8 py-6 text-sm font-bold text-cream tabular-nums">{monitor.responseTime || '—'}</td>
                <td className="px-8 py-6 text-sm font-bold text-cream tabular-nums">{monitor.uptime || 'N/A'}</td>
                <td className="px-8 py-6 text-[10px] font-bold text-rose uppercase tracking-widest">
